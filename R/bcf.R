...
bcf <- function(X_train, Z_train, y_train, pi_train = NULL, X_test = NULL, Z_test = NULL, pi_test = NULL, 
                ...
                q = 0.9, sigma2_init = NULL, num_trees_mu = 200, num_trees_tau = 50, num_gfr = 5, 
                num_burnin = 0, num_mcmc = 100, sample_sigma_global = T, sample_sigma_leaf_mu = T, 
                sample_sigma_leaf_tau = T, propensity_covariate = "mu", random_seed = -1) {
    ...
    # Add propensity score to covariate set, estimating if need be
    ...
    if (is.null(sigma_leaf_tau)) sigma_leaf_tau <- var(resid_train)/(num_trees_tau)
    ...
    forest_dataset_tau_train <- createForestDataset(X_train_tau, Z_train)
    if (has_test) forest_dataset_mu_test <- createForestDataset(X_test_mu)
    if (has_test) forest_dataset_tau_test <- createForestDataset(X_test_tau, Z_test)
    ...
    tau_hat_train <- forest_samples_tau$predict(forest_dataset_tau_train)*y_std_train
    if (has_test) {
    ...
    model_params <- list(
        ...
        "nu" = nu,
        "lambda" = lambda, 
        "sigma_leaf_mu_init" = sigma_leaf_mu,
        "sigma_leaf_tau_init" = sigma_leaf_tau,
        "a_leaf_mu" = a_leaf_mu, 
        "b_leaf_mu" = b_leaf_mu,
