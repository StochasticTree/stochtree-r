...
# Demo 6: Nonlinear Outcome Model, Heterogeneous Treatment Effect with Additive Random Effects and Forest-Specific Feature Subsetting

Suppose we believed, based on past research and domain knowledge, that only a subset of available "control variables" were effect modifiers (variables that govern the treatment effect function $\tau(X)$). We can instruct `bcf()` to use a subset of the variable we pass in `X_train` for the treatment effect forest using the `treatment_forest_vars` argument. In the ensuing simulated DGP, we know that only variables 2 and 4 modify $\tau(X)$, so in this demo we will show how to direct BCF to consider only these variables in the treatment effect forest.

*(Note: there is also a `prognostic_forest_vars` argument that enables the same subsetting process for the prognostic (control function) forest.)*

## Simulation

We reuse the data drawn in Demo 5 above.
...
bcf_model_warmstart <- bcf(
    ...
    group_ids_train = group_ids_train, rfx_basis_train = rfx_basis_train, 
    X_test = X_test, Z_test = Z_test, pi_test = pi_test, group_ids_test = group_ids_test,
    rfx_basis_test = rfx_basis_test, ordered_cat_vars = c(4,5), treatment_forest_vars = c(2,4), 
    num_gfr = num_gfr, num_burnin = num_burnin, num_mcmc = num_mcmc, 
    sample_sigma_leaf_mu = T, sample_sigma_leaf_tau = F
)
```
