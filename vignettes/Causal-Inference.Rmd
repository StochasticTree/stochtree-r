...
mu_X <- (
    ((0 <= X[,1]) & (0.25 > X[,1])) * (-7.5*X[,p_X]) + 
    ((0.25 <= X[,1]) & (0.5 > X[,1])) * (-2.5*X[,p_X]) + 
    ((0.5 <= X[,1]) & (0.75 > X[,1])) * (2.5*X[,p_X]) + 
    ((0.75 <= X[,1]) & (1 > X[,1])) * (7.5*X[,p_X])
)
tau_X <- 0.5*X[,2]
# tau_X <- rep(0,n)
f_XX <- mu_X + tau_X*Z
...
```{r regular_bart_external_pointers}
# Data
forest_dataset_bart <- createForestDataset(cbind(X,pi_X))
outcome_model_type_bart <- 0
outcome_bart <- createOutcome(resid)

# Random number generator (std::mt19937)
rng_bart <- createRNG(random_seed)

# Sampling data structures
forest_model_bart <- createForestModel(forest_dataset_bart, feature_types_mu, num_trees_mu, n, alpha_mu, beta_mu, min_samples_leaf_mu)

# Container of forest samples
forest_samples_bart <- createForestContainer(num_trees_mu, 1, T)
```

Prepare to run the sampler

```{r regular_bart_sampling_container}
global_var_samples_bart <- c(global_variance_init, rep(0, num_samples))
leaf_scale_samples_bart <- c(tau_init_mu, rep(0, num_samples))
```
...
if (num_warmstart > 0){
for (i in 1:num_warmstart) {
    forest_model_bart$sample_one_iteration(forest_dataset_bart, outcome_bart, forest_samples_bart, rng_bart, feature_types_mu, 
                                      outcome_model_type_bart, leaf_prior_scale_mu, var_weights_mu, 
                                      global_var_samples_bart[i], cutpoint_grid_size_mu, gfr = T)
    global_var_samples_bart[i+1] <- sample_sigma2_one_iteration(outcome_bart, rng_bart, nu, lambda)
    leaf_scale_samples_bart[i+1] <- sample_tau_one_iteration(forest_samples_bart, rng_bart, a_leaf_mu, b_leaf_mu, i-1)
    leaf_prior_scale_mu[1,1] <- leaf_scale_samples_bart[i+1]
}   
}
```
...
for (i in (num_warmstart+1):num_samples) {
    forest_model_bart$sample_one_iteration(forest_dataset_bart, outcome_bart, forest_samples_bart, rng_bart, feature_types_mu, 
                                      outcome_model_type_bart, leaf_prior_scale_mu, var_weights_mu, 
                                      global_var_samples_bart[i], cutpoint_grid_size_mu, gfr = F)
    global_var_samples_bart[i+1] <- sample_sigma2_one_iteration(outcome_bart, rng_bart, nu, lambda)
    leaf_scale_samples_bart[i+1] <- sample_tau_one_iteration(forest_samples_bart, rng_bart, a_leaf_mu, b_leaf_mu, i-1)
    leaf_prior_scale_mu[1,1] <- leaf_scale_samples_bart[i+1]
}
...
```{r regular_bart_predict}
# Forest predictions
bart_preds <- forest_samples_bart$predict(forest_dataset_bart)*y_std + y_bar

# Global error variance
sigma_samples_bart <- sqrt(global_var_samples_bart)*y_std
```

Inspect the BART results

```{r regular_bart_plot}
plot(sigma_samples_bart[(num_warmstart+1):num_samples], ylab="sigma")
plot(rowMeans(bart_preds[,(num_warmstart+1):num_samples]), y, pch=16, cex=0.75, xlab = "pred", ylab = "actual", main = "predicted"); abline(0,1,col="red",lty=2,lwd=2.5)
sqrt(mean((rowMeans(bart_preds[,(num_warmstart+1):num_samples]) - y)^2))
```
...
legend("bottomright", legend = c("b_0", "b_1"), col = c("blue", "orange"), pch = c(1,1))
```
